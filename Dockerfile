...
RUN /local/bin/pyston/bin/scons -j$(nproc) platform=linuxbsd target=template_release production=yes arch=x86_64 ${BUILD_FLAGS}

# Build Godot debug template for Linux
RUN /local/bin/pyston/bin/scons -j$(nproc) platform=linuxbsd target=template_debug arch=x86_64 ${BUILD_FLAGS}

# Build Godot editor for Linux
RUN /local/bin/pyston/bin/scons -j$(nproc) platform=linuxbsd target=editor arch=x86_64 ${BUILD_FLAGS}
...
RUN /local/bin/pyston/bin/scons -j$(nproc) platform=windows  target=template_release production=yes arch=x86_64

# Build Godot debug template for Windows
RUN /local/bin/pyston/bin/scons -j$(nproc) platform=windows  target=template_debug arch=x86_64

# Build Godot editor for Windows
RUN /local/bin/pyston/bin/scons -j$(nproc) platform=windows target=editor production=yes tools=yes arch=x86_64
